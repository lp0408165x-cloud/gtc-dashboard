...
const AIChatWidget = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const assistantMsg = {
        role: 'assistant',
        content: response.reply || response.message || '抱歉，暂时无法回复，请稍后重试。',
      };
      ...
      const errorMsg = {
        role: 'assistant',
        content: '网络异常，请稍后重试。如需紧急帮助，请发送邮件至 support@gtc-ai-global.com',
      };
  ...
  return (
    <>
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[520px] bg-gtc-navy border border-white/10 rounded-2xl shadow-2xl shadow-black/50 flex flex-col z-50 overflow-hidden">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {sending && (
              <div className="flex gap-2.5">
                <div className="w-7 h-7 bg-gtc-gold/20 rounded-lg flex items-center justify-center flex-shrink-0">
                  <Bot className="w-4 h-4 text-gtc-gold" />
                </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {quickQuestions.map((q) => (
                  <button
                    ...
                    className="px-2.5 py-1 text-xs bg-white/5 text-gray-400 rounded-lg hover:bg-white/10 hover:text-white transition-all"
                  >
                    {q}
                  </button>
                ))}
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 rounded-full shadow-xl flex items-center justify-center transition-all z-50 ${
          isOpen
            ...
            : 'bg-gtc-gold hover:bg-gtc-gold/90 hover:scale-105'
        }`}
      >
        {isOpen ? (
          ...
          <>
            ...
            {hasUnread && (
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-gtc-navy" />
            )}
          </>
        )}
      </button>
